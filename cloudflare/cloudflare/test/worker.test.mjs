...
t('有効行は1', db.prepare("select count(*) c from submissions where status='ok'").get().c===1);
...
const H = JSON.parse(fs.readFileSync('../src/heroes-min.json','utf8'));
...
t('第16世代 n=40 公開', s16.n===40 && s16.published===true, 'n='+s16.n);
...
t('GET /v1/stats/16 が KV から返る', r.status===200 && r.body.n===40);
